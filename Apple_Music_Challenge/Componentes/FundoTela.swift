...
struct FundoTela: View {
    ...
    var body: some View {
        ...
        Rectangle()
            .fill(
                LinearGradient(colors: color, startPoint: .top, endPoint: .center)
                )
            .onAppear {
    ...
    private func updateFor(image: UIImage) {
        let downSizeImage = dowsize(image: image)
        let colors = extractColors(image: downSizeImage)
        if let animation, !self.color.isEmpty {
    ...
    private func dowsize(image: UIImage) -> UIImage{
    ...
    private func extractColors(image: UIImage) -> [Color] {
        ...
        for index in 0..<count {
            let cropRect = CGRect(
                x: extent.origin.x,
                y: extent.origin.y + (titleHeight * CGFloat(index)),
